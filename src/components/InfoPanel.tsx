...
export default function StarMap({ active }: { active: boolean }) {
  ...
  // Background stays enabled; the user has no button to turn it off.
  ...
  useEffect(() => {
    ...
    const move = (e: PointerEvent) => {
      ...
      if (pinch.size === 2) {
        ...
        if (pinchDist) {
          zoomAt(pinchDist / d, (a.x + b.x) / 2, (a.y + b.y) / 2);
        }
          ...
          >
            ...
            <div className="pointer-events-auto absolute right-3 bottom-3 flex items-center gap-2">
              <div className="flex overflow-hidden rounded-md border border-white/10 bg-black/50 backdrop-blur-md">
                <button
                  onClick={() => zoomAt(0.66)}
                  className="px-2.5 py-1 text-white/70 hover:bg-white/10 hover:text-white"
                >
                  +
                </button>
